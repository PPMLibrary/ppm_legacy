      ...
      !  Revisions    :
      !-------------------------------------------------------------------------
      !  $Log: ppm_topo_mkfield_cmp_find_neigh.f,v $
...
#include "ppm_define.h"
      ...
      ! ---
      INTEGER                           :: k
      INTEGER, DIMENSION(:,:), POINTER  :: ineighold
      INTEGER, DIMENSION(:  ), POINTER  :: nneighold
      INTEGER, DIMENSION(:  ), POINTER  :: gaga,gagaold
      INTEGER, DIMENSION(:  ), POINTER  :: gagaind,gagaindold
      REAL(MK)                          :: t1,t2
      ! ---
      INTEGER                           :: isub,minbox
      ...
      NULLIFY(ineighold,nneighold,gagaind,gagaindold)
     ...
     &                    min_sub,max_sub,nsubs,nneigh,ineigh,info)
      ...
      WRITE(*,*) 'new find neigh took: ',t2-t1
      WRITE(46,'(I8,E20.4)') nsubs,t2-t1
      IF (info.NE.0) THEN
          info = ppm_error_error
          CALL ppm_error(ppm_err_sub_failed,'ppm_topo_mkfield',  &
     &        'Finding neighbors failed',__LINE__,info)
      ...
      CALL ppm_util_time(t1)
      CALL ppm_find_neigh_old(min_phys,max_phys,bcdef, &
     &                    min_sub,max_sub,nsubs,nneighold,ineighold,info)
      CALL ppm_util_time(t2)
      WRITE(*,*) 'old find neigh took: ',t2-t1
      WRITE(47,'(I8,E20.4)') nsubs,t2-t1
      IF (info.NE.0) THEN
          info = ppm_error_error
          CALL ppm_error(ppm_err_sub_failed,'ppm_topo_mkfield',  &
     &        'Finding neighbors failed',__LINE__,info)
      ...
      ! compare the results
      DO i=1,nsubs
          IF (nneigh(i) .NE. nneighold(i)) THEN
              WRITE(*,*) 'Difference in nneigh ',i,' is: ',nneigh(i),nneighold(i)
          ENDIF
      ENDDO
      DO i=1,nsubs
          ALLOCATE(gaga(nneighold(i)),gagaold(nneighold(i)))
          DO j=1,nneighold(i)
              gaga(j) = ineigh(j,i)
              gagaold(j) = ineighold(j,i)
          ENDDO
          CALL ppm_util_qsort(gaga,gagaind,info)
          CALL ppm_util_qsort(gagaold,gagaindold,info)
          j = 0
          DO k=1,nneighold(i)
              IF (gaga(gagaind(k)) .NE. gagaold(gagaindold(k))) THEN
                  j = 1
              ENDIF
          ENDDO
          IF (j.GT.0) THEN
              PRINT*,i,'new: ',gaga(gagaind)
              PRINT*,i,'old: ',gagaold(gagaindold)
          ENDIF
          DEALLOCATE(gaga,gagaold)
      ENDDO
      DEALLOCATE(gagaind,gagaindold)
