        ...
        DO isub=1,nsubs
           DO k=start(3,isub,1),stop(3,isub,1)
              DO j=start(2,isub,1),stop(2,isub,1)
                DO i=start(1,isub,1),stop(1,isub,1)
                 DO ilda=1,vecdim
                   res  = (16.0_MK*u(ilda,i-1,j,k,isub)+&
     &                      16.0_MK*u(ilda,i+1,j,k,isub)-&
     &                        u(ilda,i-2,j,k,isub)-u(ilda,i+2,j,k,isub))*c22 +&
     &                        (16.0_MK*u(ilda,i,j-1,k,isub)+&
     &                         16.0_MK*u(ilda,i,j+1,k,isub)-&
     &                         u(ilda,i,j-2,k,isub)-u(ilda,i,j+2,k,isub))*c33+&
     &                        (16.0_MK*u(ilda,i,j,k-1,isub)+&
     &                         16.0_MK*u(ilda,i,j,k+1,isub)-&
     &                         u(ilda,i,j,k-2,isub)-u(ilda,i,j,k+2,isub))*c44-&
     &                         u(ilda,i,j,k,isub)*c55-f(ilda,i,j,k,isub)
